...
for the distribution of timing signals to all the DUNE detector components and for issuing
the configuration that determines data taking in the active readout components. The
versioned DAQ configuration model is maintained in the DUNE-DAQ framework, including the
DAPHNE definitions in \texttt{daphnemodules}. At run time DAQ applies that configuration
using its authenticated role on the shared PDS OPC-UA gateway. Slow Control (SC) uses a
separate authenticated role on the same gateway to configure and maintain the equipment
services needed to keep each endpoint powered, available, monitored, and protected. SC
does not receive, approve, modify, or execute a DAQ configuration handoff. The fixed
ownership split and shared gateway interface are described in
Section~\ref{subsec:configuration-path}.
...
DAQ/PDS owns the pulse parameters, timing-group selection, DAPHNE capture configuration,
and execution of a data-taking calibration sequence. SC owns LCM external power, service
availability, environmental monitoring, equipment alarms, and protective inhibit or
safe-state actions. SC may monitor the requested and effective calibration settings without
acquiring write authority over them.

\subsection{Timing interface}

\label{sec:systems-timing}
